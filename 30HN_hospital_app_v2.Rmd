---
title: "30HN - DOT1000 in Hospital "
runtime: shiny
output: 
...
# pageWithSidebar(
fluidPage(
  headerPanel(h2("Upload file", align = "center")),
  # sidebarPanel(
  fluidRow(
    column(
      ### hospital selection 
      selectInput("hospital", label = h3("Please choose the hospital"), choices = c("BVDK Nam Dinh"="Nam Dinh hospital", "BVDK Nghia Hung"="Nghia Hung hospital", "BVDK Xuan Truong"="Xuan Truong hospital", "BVDK Hai Hau"="Hai Hau hospital")),
  ...
  # mainPanel(
    ...
    column(4,
           # patient 
           fileInput(inputId = "patient_upload",label = h3("Upload the data of patient"),accept = c(".csv",".Rds",".xlsx",".xls",".xlsm"),multiple = TRUE)),
    column(4,
           # drug
           fileInput(inputId = "drug_upload",label = h3("Upload the data of drug"),accept = c(".csv",".Rds",".xlsx",".xls",".xlsm"),multiple = TRUE)),
    column(4,
           # bed 
           fileInput(inputId = "bed_upload",label = h3("Upload the data of bed"),accept = c(".csv",".Rds",".xlsx",".xls",".xlsm"),multiple = TRUE)
    # )
    )
...
classified_drug <- reactive({
  ...
  classified_drug <- drug %>% 
    ...
    left_join(.,ab, by=c("ten_thuoc", "duong_dung"),multiple = "all") %>%
...
Monthly DOT1000 by ward 
=======================================================================


```{r}
# input IU
selectInput("ward","Please select ward to visualise", "")
...
output$ward_dot_plot <- renderPlotly({
  ...
  DOT_1000_each_ab <- DOT_assignw %>%
   ...
   plot_ly(data = DOT_1000_each_ab) %>% add_trace( x = ~date, y= ~DOT_1000_sum,color = ~atc_level_name, type = "scatter",mode = "lines+markers") %>% layout(
      title = paste("DOT1000 monthly for ward:",input$ward,"of",input$hospital),
      xaxis = list(title = "Time (Month)"),
      yaxis = list(title = "DOT1000")
                              )
...
Monthly DOT1000 by ICD10
=======================================================================

```{r}
fluidPage(
  
  headerPanel(h2("DOT1000 by ICD10 Chapter or you can specify multiple ICD10 code",align = "center"))
  
  , fluidRow(column(12,
    
    selectizeInput(
      "icd10"
      , "Enter one or multiple ID10 codes"
      , choices = NULL
      , multiple = TRUE
...
output$icd_dot <- renderPlotly({
  ...
  DOT_1000_chapter <- DOT_assignw %>%
   ...
   plot_ly(data = DOT_1000_chapter) %>% add_trace( x = ~date, y= ~DOT_1000_sum,color = ~Chapter, type = "bar", colors = col_pal) %>% layout(
      title = paste("DOT1000 monthly for ",input$hospital,"per ICD10 chapter"),
      xaxis = list(title = "Time (Month)"),
      yaxis = list(title = "DOT1000"),
      barmode = "stack"
                              )
...
Row {data-width=500}
-----------------------------------------------------------------------
### DOT1000 by selected ICD10 codes
...
output$byicd_dot <- renderPlotly({
  ...
  print(paste("There are ",nrow(DOT_assignw), "records found from the selected records"))
  # logical condition
   observeEvent(input$icd10,{
     if(nrow(DOT_assignw) == 0){
      showNotification("No record found with selected ICD10 codes")
     }
    })
  ...
  DOT_1000_icd10 <- DOT_assignw %>%
   ...
   plot_ly(data = DOT_1000_icd10) %>% add_trace( x = ~date, y= ~DOT_1000_sum,color = ~icd10, type = "bar") %>% layout(
      title = paste("DOT1000 monthly for ",input$hospital,"per selected ICD10"),
      xaxis = list(title = "Time (Month)"),
      yaxis = list(title = "DOT1000"),
      barmode = "stack"
   )
...
Monthly DOT1000 by AWaRe category
=======================================================================


Row {data-width=500}
-----------------------------------------------------------------------
### DOT1000 by AWaRe category
...
output$aware_dot <- renderPlotly({
  ...
  DOT_1000_aware <- DOT_assignw %>%
   ...
   plot_ly(data = DOT_1000_aware) %>% add_trace( x = ~date, y= ~DOT_1000_sum,color = ~Category, type = "bar", colors = col_pal) %>% layout(
      title = paste("DOT1000 monthly for ",input$hospital,"follow AWaRe category"),
      xaxis = list(title = "Time (Month)"),
      yaxis = list(title = "DOT1000"),
      barmode = "stack"
   )
